    ...
    try:
        ...
        try:
            ...
            if objson != '':
                ...
                obJsonText = "{\"from\":\"hitokoto.cn\",\"text\":\"" + \
                    txt + "\",\"anthor\":\"" + author + "\"}"
                saveToFile(obJsonText)
        except:
            print("OneLineDecodeError")
    ...
    try:
        ...
        try:
            ...
            if objson != '':
                ...
                obJsonText = "{\"from\":\"hitoapi.cc\",\"text\":\"" + \
                    txt + "\",\"anthor\":\"" + author + "\"}"
                saveToFile(obJsonText)
        except:
            print("hitoApiDecodeError")
    except ReadTimeout:
...
def getSatoriHitokoto():
    try:
        get = requests.get(
            "https://api.satori.moe/hitokoto.php", headers=headers, timeout=2)
        try:
            objson = json.loads(get.text)
            if objson != '':
                txt = objson["hitokoto"].replace('\n', '')
                ...
                obJsonText = "{\"from\":\"satori.moe\",\"text\":\"" + \
                    txt + "\",\"anthor\":\"" + author + "\"}"
                saveToFile(obJsonText)
        except:
            print("SatoriDecodeError")
    except ReadTimeout:
